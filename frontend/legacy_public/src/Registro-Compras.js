...
async function apiGetMateriales() {
  return fetchJson(API_MATERIALES);
}

async function apiGetCompras() {
  return fetchJson(API_COMPRAS);
}
...
const DB = {
  ...
  nextCompraId() {
    const n = parseInt(localStorage.getItem(LS.idCompra) || "1", 10);
    localStorage.setItem(LS.idCompra, String(n + 1));
    return n;
  },
...
function renderFinderList(items) {
  ...
  for (const L of letras) {
    ...
    for (const it of groups[L]) {
      ...
      rows.push(`
        <div class="fi-item${active}" data-id="${it.id}">
...
function bindFinder() {
  ...
  Finder.els.input.addEventListener("focus", () => {
    if (!Finder.els.input.value.trim()) openFinder(Finder.data, "");
  });
  ...
  Finder.els.input.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      Finder.index = Math.min(itemCount - 1, Finder.index + 1);
      renderFinderList(Finder.viewItems);
    ...
    if (e.key === "ArrowUp") {
      e.preventDefault();
      Finder.index = Math.max(0, Finder.index - 1);
      renderFinderList(Finder.viewItems);
      scrollActiveIntoView();
    }
    ...
    if (e.key === "Enter") {
      e.preventDefault();
      const chosen = Finder.viewItems[Finder.index] || Finder.viewItems[0];
      if (chosen) selectFinderId(chosen.id);
    }
  ...
  document.addEventListener("mousedown", (e) => {
    if (Finder.open && !Finder.els.root.contains(e.target)) closeFinder();
  });
}
...
function renderTabla() {
  ...
  const rows = state.pendientes.slice().sort(
    (a, b) => b.fechaISO.localeCompare(a.fechaISO) || a.nombre.localeCompare(b.nombre)
  );
...
function editarFilaCantidad(tr, idMat) {
  const celda = tr.querySelector(".qty-cell");
  if (!celda || celda.classList.contains("editing")) return;

  const pend = state.pendientes.find(p => p.id_Material === idMat);
  if (!pend) return;
  ...
  okBtn.addEventListener("click", () => {
    ...
    renderTabla();
    notify("Cantidad actualizada.", "ok");
  });

  noBtn.addEventListener("click", () => {
    celda.classList.remove("editing");
    celda.textContent = orig;
  });
...
function eliminarFila(tr, idMat) {
  ...
  state.pendientes.splice(i, 1);
  DB.setPend(state.pendientes);
  tr.remove();

  if (!state.pendientes.length) renderTabla();

  notify("Material eliminado.", "warn");
  updateActionStates();
}
...
function onPlusQty(e) {
  ...
  const inp = $("#cantidad");
  if (!inp) return;
  inp.value = String(clampInt(inp.value, 1) + 1);
  updateActionStates();
}
...
function onCantidadInput() {
  ...
  updateActionStates();
  if (!(cant > 0)) {
    notify("Poner cantidad de material", "warn");
  }
}
...
function onAgregar(e) {
  ...
  if (!idMat) {
    notify("Selecciona un material.", "warn");
  ...
  if (!mat) {
    notify("Material no encontrado.", "danger");
    return;
  }

  const i = state.pendientes.findIndex(p => Number(p.id_Material) === idMat);
  if (i >= 0) {
...
async function onGuardar(e) {
  ...
  try {
    ...
    let matsResp = null;
    try {
      matsResp = await apiGetMateriales();
    } catch {}

    if (matsResp?.success && Array.isArray(matsResp.data)) {
      DB.setMats(matsResp.data);
      state.materiales = matsResp.data.slice();
      buildFinderData();
    ...
    for (const it of items) {
      const m = mats.find(mm => Number(mm.id_Material) === Number(it.id_Material));
    ...
    sessionStorage.setItem(UI_SAVED_THIS_SESSION, "1");
    updateActionStates();
    notify("Compra guardada local (sin BD).", "warn");
  }
}
...
async function onCancelar(e) {
  ...
  if (!state.pendientes.length) {
    notify("No hay nada que cancelar.", "warn");
    return;
  }
...
async function init() {
  ...
  $("#btnVerCompras")?.addEventListener("click", (e) => {
    e.preventDefault();
    if (!e.currentTarget.disabled) location.href = "./compras.html";
  });
